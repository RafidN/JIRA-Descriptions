...
REPOS_FILE = Path(__file__).parent.parent / "repos.txt"

BASE_DIR = Path(__file__).parent.parent / "cloned_repos"
...
def check_repos(url: str) -> Path:
    ...
    try:
        if url in local_repos:
            ...
            Repo.clone_fromurl(url, local_path, depth=1)
            append_repo(url)
    except GitCommandError as e:
        logger.exception(f"Git operation failed for {url}")
        raise HTTPException(status_code=400, detial=f"Failed to clone or pull repo: {e}")
